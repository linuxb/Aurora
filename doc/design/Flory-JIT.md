# **Flory JIT Dynamic Graph Expansion Design**

Flory JIT lets Aurora continue planning when an initial DAG contains a Planner Node that cannot map directly to an existing Skill. The runtime expands that node into a concrete child DAG, hot-plugs the child graph into the scheduler, and continues execution without rebuilding the whole session.
...
export async function runPlanner(ctx: PlannerContext): Promise<PlannerResult> {
  ...
  // 2. Build a ReAct prompt and call Flory's internal LLM Proxy.
  const prompt = `
...
## **4. Hot-Plugging: Flory Transaction Engine**

When Flory receives `SUCCESS_AND_EXPAND`, graph expansion must be atomic and must not leave broken edges. Because TiDB supports distributed transactions, Flory executes the mutation as one transaction.
...
Letting an Agent write loops and subgraphs automatically is dangerous. Flory must add three layers of guardrails at the metadata layer.
...
Before Flory starts the DB transaction for an expansion request, it validates:
...
2. **Budget circuit breaker**
   - Flory accumulates `budget_consumed` from proxy billing callbacks.
...
3. **Human-in-the-loop yield**
   ...
   - If the expansion payload contains `Ask_Human`, Flory performs a controlled suspend.
...
Depth alone does not express repeated inability to map work to a concrete Skill. Flory should track `jit_unmapped_streak` at the DAG or planner-chain level.
