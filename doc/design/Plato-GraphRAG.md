...
Plato 是 Aurora 架构中 Polaris 记忆系统的高阶抽象层（GraphRAG 子系统）。
...
在检索阶段，Arqo (Go) 调度器内部的 Planner 会根据当前任务的目标，生成包含查询代价预判的 mem_hint。Plato 接收到 mem_hint 后，执行物理级的查询分流。
...
* **Plato 执行 (Map-Reduce)**：  
  1. **Map**：跳过底层成千上万的繁杂微观节点，直接在图库中检索 Community 节点。通过向量相似度或关键词，找出与 intent_question 最相关的 Top-3 社区的 macro_summary。  
